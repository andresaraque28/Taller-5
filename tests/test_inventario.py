...
def test_sin_ventas_dura_infinito():
    ...
    assert dias_de_inventario(50, 0) == 1.0 # modificacion de -1 al 1 para inducir al


def test_necesita_reposicion_true():
    # 20 unidades, 10 por día -> 2 días < 7 -> sí necesita
    assert necesita_reposicion(20, 10) is False # valor original True

def test_no_necesita_reposicion():
    # 100 unidades, 10 por día -> 10 días > 7 -> no necesita
    assert necesita_reposicion(100, 10) is True # Valor original False
